...
public class IfcOWLUtils extends ModelUtils {
  ...
  /**
   * Returns the property position of an property (e.g. GlobalId) according to ifcOWL ontology and
   * ifcOWL supplyment ontology
   *
   * @param p property resource
   * @param schema schema model (with both ifcOWL ontology and corresponding ifcOWL supplyment
   *     ontology)
   * @return
   */
  public static int getPropertyPosition(Property p, Model schema) {
    Resource clazz =
        schema.listStatements(p, RDFS.domain, (RDFNode) null).next().getObject().asResource();
    Property index = schema.getProperty(IfcOWLSupplement.getURI() + "index");
    int i = p.listProperties(index).next().getInt();
    Property isTopLevelIfcEntity =
        schema.getProperty(IfcOWLSupplement.getURI() + "isTopLevelIfcEntity");
  ...
  private static Set<Resource> getAllSubClasses(
      Resource superClass, Model model, HashSet<Resource> hashSet) {
  ...
  private static boolean hasSuperClass(
      Resource subClass, Resource superClass, Set<Resource> reached) {
  ...
  private static void recurGetProperties(
      HashMap<Resource, List<Property>> hashMap,
      Resource clazz,
      Model schema,
      Property index,
      Property ifcSubClassOf,
      List<Property> inherited) {
  ...
  private static List<Resource> getSubClasses(
      Resource superClass, Model model, Property ifcSubClassOf) {
    List<Resource> classes = new ArrayList<Resource>();
